class EnglishTest {
    ...
    async loadQuestions() {
        try {
            const response = await fetch('etest.json');
            if (!response.ok) {
    ...
    showResults() {
        ...
        this.scoreValue.textContent = `${results.score}/${results.total}`;
    ...
    displayIncorrectAnswers(incorrectQuestions) {
        ...
        incorrectQuestions.forEach((item, index) => {
            ...
            // UPDATED: Show ALL options with appropriate highlighting
    ...
    restartTest() {
        ...
        // Show question section and hide results
